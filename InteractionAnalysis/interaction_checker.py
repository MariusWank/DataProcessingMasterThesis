...
def main(root, x_divider, x_ticks_labels, red_blue_divider, linestyle_divider, categories, enable_legend):
    ...
    for i, category in enumerate(categories):
        ...
        plt.xlabel("Branch depth dependency in µm")
        plt.ylim(bottom=0)
        ...
        legend_elements_colors = [
            Line2D([0], [0], color="#74B6CF", lw=2, label=r'$k_{1} = 0.1$'),
            Line2D([0], [0], color='#FF6961', lw=2, label=r'$k_{1} = 0.3$')
        ]
        ...
        if enable_legend:
            ...
            first_legend = plt.legend(handles=legend_elements_colors, loc='upper center', bbox_to_anchor=(0.5, -0.1),
                                      fancybox=True, shadow=True, ncol=2)

            # Add the second legend below the first one
            second_legend = plt.legend(handles=legend_elements_styles, loc='upper center', bbox_to_anchor=(0.5, -0.2),
...
if __name__ == "__main__":
    ...
    x_ticks_labels = ["20", "40", "60", "80", "100"]
